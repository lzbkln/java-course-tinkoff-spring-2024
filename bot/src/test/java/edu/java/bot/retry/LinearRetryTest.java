...
import static edu.java.bot.configuration.retry.RetryType.LINEAR;
...
public class LinearRetryTest {
    ...
    private static void properties(DynamicPropertyRegistry registry) {
        registry.add("app.retry.type", () -> LINEAR);
        registry.add(
    ...
    @Test
    @DisplayName("Test that linear retry works correct for client error")
    void testThatLinearRetryWorksCorrect() {
    ...
    @Test
    @DisplayName("Test that linear retry works correct for server error")
    void testThatLinearRetryWorksCorrectForServerError() {
        wireMockServer.stubFor(WireMock.post(TG_CHAT.formatted(1L))
            .willReturn(aResponse()
                .withStatus(500)));
